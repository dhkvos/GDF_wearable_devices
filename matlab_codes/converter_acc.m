...
fid =fopen('/Users/jenny/Desktop/4signals/Acc.csv');
for i=1:5, buffer=fgetl(fid); end
m = textscan(fid,'%f%f%f%f%f%f%f%f%f','delimiter',',');
x = [m{:}];

filename = '/Users/jenny/Desktop/new/Acc_11';
column_label = {'Time';'X';'Y';'Z';'|V|';'MAx';'MAy';'MAz';'MAv'}; % channel identification, max 80 char. per channel
sample_rate = 100; % sampling frequencyt in [Hz]
units = {'?';'?';'?';'?';'?';'?';'?';'?';'?'};  % physical dimesions
start_time = [2021 02 19 15 16 0.00];


GDFTYP = 16*ones(1,size(x,2)); % define datatypes (GDF only, see GDFDATATYPE.M for more details)
PhysMax = [9999;9999;9999;9999;9999;9999;9999;9999;9999];
PhysMin = [-9999;-9999;-9999;-9999;-9999;-9999;-9999;-9999;-9999];
